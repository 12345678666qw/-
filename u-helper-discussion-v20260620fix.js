...
(function () {
    ...
    async function fillTextarea(textarea, text) {
        try {
            ...
            // 🔧 页面上下文派发 InputEvent — React isTrusted/instanceof 检查才通过
            var marker = 'uh-' + Math.random().toString(36).slice(2, 8);
            textarea.setAttribute('data-uh-marker', marker);
            var script = document.createElement('script');
            script.textContent =
                '(function(m){' +
                'var el=document.querySelector("[data-uh-marker="+m+"]");' +
                'if(el){' +
                'el.dispatchEvent(new InputEvent("input",{bubbles:true,inputType:"insertText",data:el.value,cancelable:true}));' +
                'el.dispatchEvent(new Event("change",{bubbles:true}));' +
                '}' +
                '})(' + JSON.stringify(marker) + ')';
            document.documentElement.appendChild(script);
            if (script.parentNode) script.parentNode.removeChild(script);
            textarea.removeAttribute('data-uh-marker');

            await sleep(200);
            // 不调 blur: U校园 handleBlurReply → setState({replyInfo:null}) 会重置 state
    ...
    function findSubmitButton(root) {
        ...
        var scopedSelectors = [
            '.ds-discussion-bottom-btns button',
            '.ds-discussion-bottom button',
    ...
    async function handleGenericCommentSection() {
        try {
            ...
            // 🔧 页面上下文派发 InputEvent
            var _marker2 = 'uh-gc-' + Math.random().toString(36).slice(2, 8);
            commentTextArea.setAttribute('data-uh-marker', _marker2);
